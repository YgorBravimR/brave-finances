...
import { categoriesArray, creditCardsArray, invoicesArray, tagsArray } from '../../../../../utils/transactionts';
...
export function CreditCardTransactionForm() {
  ...
  const [card, setCard] = useState(creditCardsArray[0].value)
  ...
  const handleChangeCardSelect = (event: SelectChangeEvent) => {
    const { value } = event.target
    formik.setFieldValue("card_name", value)
    setCard(value as string);
  };
  ...
  const formik = useFormik({
    initialValues: {
      ...
      card_name: creditCardsArray[0].value,
        ...
        <FormControl>
          ...
          >
            {creditCardsArray.map((card) => (
              <MenuItem key={card.title} value={card.value}>{card.title}</MenuItem>
            ))}
          </Select>
        </FormControl>
        <FormControl>
